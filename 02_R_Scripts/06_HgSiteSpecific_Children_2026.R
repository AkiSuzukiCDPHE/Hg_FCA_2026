...
Hg_Statewide_long2 <- Hg_Statewide_long |>  mutate(Children_Statewide_Per_Month = case_when(Statewide_Character =="1 meal/week" ~ 4,
...
Hg_SS5 <- Hg_SS4 %>%
  ...
  mutate(
    # Handle cases where a waterbody/species combo wasn't in the reference data at all
    Children_State_Status = ifelse(is.na(Children_State_Status), "No", Children_State_Status)
  )
